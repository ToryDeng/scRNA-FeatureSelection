markergene.cellassign <- function(data, n_genes, file_name){
  require(magrittr)
  require(limma)
  ...
  marker_gene_mat <- expr_mat_thres[(maxdiffs >= nth_diff) ,] %>%
    as.matrix
  write.csv(marker_gene_mat,stringr::str_glue("{file_name}.csv"))
  marker_gene_mat
}
...
#count_matrix = read.csv('/home/tdeng/SingleCell/data/pancreas/separated data/Segerstolpe_pancreas_filtered.csv', row.names = 1, header = T)
#rownames(test) <- sapply(strsplit(rownames(test),"\t"),"[",2)
#trueID = as.matrix(read.csv('/home/tdeng/SingleCell/data/pancreas/separated data/Segerstolpe_trueID_filtered.csv', row.names = 1))

#count_matrix <- as.matrix(t(count_matrix))
#sce <- SingleCellExperiment(assays = list(counts = count_matrix))
#colData(sce)$label <- make.names(trueID)

for (data_name in c('PBMC10', 'PBMC20')){
  print(data_name)
  scex <- get_sce(data_name)
  setwd('/home/tdeng/R')
  markergene.cellassign(scex,1000, paste(data_name,'markers', sep = '_'))
}
