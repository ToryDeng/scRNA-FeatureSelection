...
mtx<-t(counts(sce))
batch_info <- as.character(as.data.frame(colData(sce)["label"])[,1])
...
for (i in 1:length(k_neighbors)){
  batch.estimate <- kBET(mtx, batch_info, plot=FALSE, do.pca=FALSE, k0=k_neighbors[i], verbose=TRUE)
...
write.csv(res, "tempData/temp_correction.csv")
